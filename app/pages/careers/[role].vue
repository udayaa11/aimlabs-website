<template>
  <div v-if="career" class="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-[120px] py-8 sm:py-10 space-y-6">
    ...
    <NuxtLink to="/careers/career" class="group inline-flex items-center gap-2 text-gray-700 text-sm">
      <img src="/assets/svg/backToCareers.svg" alt="Back Icon" class="w-5 h-5 block group-hover:hidden" />
      <img src="/assets/svg/backArrowRed.svg" alt="Back Red Icon" class="w-5 h-5 hidden group-hover:block" />
      <span
    ...
    <div class="bg-red-600 text-white rounded-xl p-5 sm:p-6 md:p-8 space-y-3">
      ...
      <div class="flex flex-wrap gap-3 sm:gap-4 md:gap-6 text-sm md:text-base items-center">
        <div class="flex items-center gap-2">
          <img src="/assets/svg/yearsIcon.svg" alt="Experience" class="w-4 h-4" />
          <span>{{ career.experience }}</span>
        </div>
        <div class="flex items-center gap-2">
          <img src="/assets/svg/location.svg" alt="Location" class="w-4 h-4" />
          <span>{{ career.location }}</span>
        </div>
        <div class="flex items-center gap-2">
          <img src="/assets/svg/fulltime.svg" alt="Job Type" class="w-4 h-4" />
    ...
    <div class="flex flex-col gap-4 mx-auto">
      ...
      <div class="flex justify-center pt-4">
        <button @click="afStore.openFormPopup" :disabled="!allChecked"
      ...
      <transition name="fade">
        <div v-if="afStore.isFormPopupOpen"
          class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div class="bg-white rounded-xl shadow-lg p-8 max-w-lg w-full relative">
            <button @click="afStore.closeformPopup" class="absolute top-3 right-3 text-gray-500 hover:text-gray-700">
              ✕
            </button>
            <FormSubmit />
          </div>
...
import FormSubmit from "~/components/formSubmit/formSubmit.vue"
